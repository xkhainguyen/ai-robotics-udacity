...
class robot:
    ...
    def measurement_prob(self, measurement):    # used for particle

        # compute errors
        error_x = measurement[0] - self.x       # difference between a particle and robot measure
...
def run(grid, init, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000):

    myrobot = robot()
    myrobot.set(init[0], init[1], 0.)
...
#         estimate = myrobot.sense()

        robot_positions.append([estimate[0], estimate[1]])
        
        robot_positions2.append([myrobot.x, myrobot.y])
        
        filter_pos = []
        for i in range(len(filter.data)):
            filter_pos.append([filter.data[i].x, filter.data[i].y])
        particles_positions.append(filter_pos)
...
#         print(u)
        cte = (-Rx*Dy + Ry*Dx)/sqrt(Dx**2. + Dy**2.)  
        if u > 1.0:
            index += 1
...
def main(grid, init, goal, steering_noise, distance_noise, measurement_noise, 
    ...
    for i in range(len(path.spath)):
        xd.append(path.spath[i][0])
        yd.append(path.spath[i][1])
    ...
    plt.scatter(*tuple(zip(*obstacles)), s=3000, marker='o')    # adjust size parameter

    return run(grid, init, goal, path.spath, [p_gain, d_gain])
...
# shoulda used OOP: 
# fig, ax = plt.figure(figsize=(len(grid), len(grid[0])))
# ax.plot()

fig = plt.figure(figsize=(len(grid), len(grid[0])))
plt.xlim((-.5, len(grid) -.5))
plt.ylim((-.5, len(grid[0]) -.5))
plt.xticks([x-.5 for x in range(len(grid))])
plt.yticks([x-.5 for x in range(len(grid[0]))])
plt.grid()
...
ani = animation.FuncAnimation(fig, update, frames=range(len(robot_positions)), blit=True, repeat=False, interval=100)
plt.show()  
# plot_traj(xd, yd, x, y)
