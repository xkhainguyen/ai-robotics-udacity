...
def run(grid, goal, spath, params, printflag=False, speed=0.1, timeout=1000, plott=False, collisions_coefficient=100, steps_coefficient=10):
    myrobot = robot()
    myrobot.set(0., 0., 0.)
...
def main(grid, init, goal, steering_noise, distance_noise, measurement_noise,
         weight_data, weight_smooth, p_gain, d_gain, plott=False):
    ...
    if plott:
        ...
        plt.scatter(*tuple(zip(*obstacles)), s=3100, marker='s')      

    return run(grid, goal, path.spath, [p_gain, d_gain], plott=plott)
...
def twiddle_run(p, n_launches=10):
    ...
    for launch in range(n_launches):
        total_cte = main(grid, init, goal, steering_noise, distance_noise, measurement_noise, 
            weight_data, weight_smooth, p_gain, d_gain, plott=False)
        error += total_cte
...
print(main(grid, init, goal, steering_noise, distance_noise, \
           measurement_noise, weight_data, weight_smooth, p_gain, d_gain, plott=True))