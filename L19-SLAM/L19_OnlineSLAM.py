...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    # Initialize constraint information matrix and vector
    Omega = matrix()
    Omega.zero(dim, dim)
    Omega.value[0][0] = 1
    Omega.value[1][1] = 1
    ...
    for k in range(len(data)):
        ...
        # n is the index of robot position in the matrix/vector
        n = 2*k 
        ...
        for i in range(len(Z)):
            ...
            # m is the index of landmark cordinate in the matrix/vector
            m = 2*N + 2*Z[i][0]

            # update the infomation matrix/vector based on the measurement
            for b in range(2): # index for x, y
                ...
                Xi.value[n+b][0]      += -Z[i][b + 1]/measurement_noise
                Xi.value[m+b][0]      +=  Z[i][b + 1]/measurement_noise

        # update the information matrix/vector based on the motion
        for b in range(4):
            Omega.value[n+b][n+b]       +=  1./motion_noise
        for b in range(2):
            Omega.value[n+b  ][n+2+b]   += -1./motion_noise
            Omega.value[n+2+b][n+b]     += -1./motion_noise
            Xi.value[n+b  ][0]          += -dp[b]/motion_noise
            Xi.value[n+2+b][0]          +=  dp[b]/motion_noise
    ...
    for k in range(len(data)):
        ...
        for i in range(len(Z)):
            ...
            for b in range(2): # index for x, y
                ...
                Xi.value[b][0]          += -Z[i][b + 1]/measurement_noise
                Xi.value[m+b][0]        +=  Z[i][b + 1]/measurement_noise

        # update the information matrix/vector based on the motion
        ls = [0, 1] + list(range(4,dim+2))
        Omega = Omega.expand(dim + 2, dim + 2, ls, ls)
        Xi = Xi.expand(dim + 2, 1, ls, [0])
        ...
        for b in range(2):
            ...
            Xi.value[b  ][0]          += -dp[b]/motion_noise
            Xi.value[2+b][0]          +=  dp[b]/motion_noise

        # Omega.show('Omega = ')
        # Xi.show('Xi = ')
        ls = list(range(2, len(Omega.value)))
        Omega2 = Omega.take(ls)    
        Xi2 = Xi.take(ls, [0])
        A = Omega.take([0, 1], ls)   
        B = Omega.take([0, 1])
        C = Xi.take([0, 1], [0])
