...
def estimate_next_pos(measurement, OTHER = None):
    if not OTHER:
        ...
        t, pre, vector_pre, x_ls, y_ls, r_ls, dtheta_ls, theta0_ls = OTHER
        t += 1
        ...
        # print("R = ",r)
        r = 8.12
        vector = [measurement[0] - xc, measurement[1] - yc]
        ...
        if t >= 2:
            ...
            theta0 = (theta0 - t*dtheta)%(2*pi)
            theta0_ls.append(theta0)
            theta0 = sum(theta0_ls)/len(theta0_ls)
            print("theta0 = ", theta0)
            
            xy_estimate = [xc + r*cos(theta0 + dtheta*(t+1)), yc + r*sin(theta0 + dtheta*(t+1))]
        else:
            xy_estimate = measurement
        vector_pre = vector
    pre = measurement   
    
    OTHER = [t, pre, vector_pre, x_ls, y_ls, r_ls, dtheta_ls, theta0_ls]
